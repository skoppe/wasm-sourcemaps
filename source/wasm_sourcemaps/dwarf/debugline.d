...
align(1) struct LineProgramHeader32L {
  ...
  ubyte maximumOperationsPerInstruction; // dwarf 4
	...
	private void buildMachine(ref LineProgram lp, ubyte[] program) {
		...
		while (!program.empty) {
				...
				switch (opcode) with (StandardOpcode) {
					case extendedOp:
						...
						switch (eopcode) with (ExtendedOpcode) {
							...
							case setAddress:
								ulong address = program.read!uint(); // on 32bit needs to be uint, on 64bit needs to be ulong
            ...
            lp.m_addresses ~= m.toAddressInfo();
					...
					case constAddPC:
            // trace("constAddPC ", (255 - lp.m_header.opcodeBase) / lp.m_header.lineRange * lp.m_header.minimumInstructionLength);
...
auto toAddressInfo(ref Machine m) {
  return AddressInfo(m.line, m.column, m.fileIndex, m.address, m.operationIndex, m.isStatement, m.isBasicBlock, m.isEndSequence, m.isPrologueEnd, m.isEpilogueBegin);
}
